...
<<<<<<< HEAD
	// КэшФормПлагинов является структурой. Если обратиться значению по ключу, которого в структуре нет, то вызывается исключение.
	// здесь лучше использовать предварительную проверку на наличие свойства у структуры, иначе появляется ошибка, что поле не обнаружено
=======
>>>>>>> 60d7eb214365c50a1a9be038d2583c077c565b0f
...
<<<<<<< HEAD
				КонецЕсли;
				//Если ПеременнаяСодержитСвойство(КонтекстВыполнения, "ПодсистемаВстроенаВКонфигурацию") Тогда
				//	КонтекстВыполнения.ПодсистемаВстроенаВКонфигурацию = Объект.ЭтоВстроеннаяОбработка; 	
				//КонецЕсли; 	
=======
				КонецЕсли;	
>>>>>>> 60d7eb214365c50a1a9be038d2583c077c565b0f
...
&НаСервере
<<<<<<< HEAD
Функция СоздатьОбъектПлагина(Идентификатор)
	
	ОбъектНаСервере = ЭтотОбъектНаСервере();
		
	Возврат ОбъектНаСервере.СоздатьОбъектПлагина(Идентификатор);
=======
